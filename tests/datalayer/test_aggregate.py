...
def test_aggregate_price_skips_zero_prices():
    # price 0.0 = 기프트카드/플레이스홀더 → min 오해 방지 (MDA-2)
    prods = [_p(price=0.0), _p(price=150.0), _p(price=300.0)]
    agg = brand_aggregate(BrandExtractionResult(brand="b", source="shopify", products=prods))
    assert agg["price"]["min"] == 150.0
    assert agg["price"]["n"] == 2
