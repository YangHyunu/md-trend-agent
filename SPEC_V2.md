...
1. 좋은 평가를 받은 `out/report.html`을 제품의 기준 산출물로 고정한다.
...
- `report.html`은 기준 디자인이지만 empty/partial/full Report Quality Contract를 아직 통과하지 않았다.
...
6. 사용자가 승인, 수정, 취소한다.
7. 승인된 계획으로 상품, NAVER, 런웨이, 에디토리얼 데이터를 수집한다.
...
- v2 12-section Report Quality Contract와 golden
...
`expected_count`는 조사 참고값이다. 실행 커버리지 계산은 실제 `collected_count`를 사용한다. `manual_review_required`, `restricted`, `requires_login=true`, `blocked`, `paywalled`, `unknown` source는 자동 fetch 계획을 통과할 수 없다. `public_fetch_reviewed`는 운영자의 현재 수집 정책 판단을 기록하는 값이며 법적 권리 보장을 의미하지 않는다. robots와 약관 snapshot이 만료됐거나 hash가 변경되면 첫 fetch 전에 재검사하고 새 판단 전까지 source를 `manual_review_required`로 둔다. 수집 중 로그인·구독·CAPTCHA가 나타나면 즉시 중단하고 access status를 갱신한다.
...
현재 `out/report.html`은 **v1 visual reference**다. 타이포그래피, 카드 밀도, 정보 위계는 보존하되 현재 7개 section을 v2의 12개 필수 section과 동일한 golden으로 간주하지 않는다. v2 renderer가 수용 기준을 통과하면 고정 fixture, renderer version, HTML hash, screenshot을 `v2 golden`으로 별도 동결한다.

v1→v2 migration은 다음 의미 매핑을 유지한다.
...
3. Market Snapshot
4. Domestic Demand
5. Runway & Editorial Direction
6. Brand Signatures / Design Map
7. Item, Color, Material, Silhouette, Price
8. Steady Signals
9. Gaps, Opportunities, Risks
10. MD Actions
11. Limitations & Failures
...
- v2 12-section full/partial/empty golden과 action evidence 일치가 통과한다.
