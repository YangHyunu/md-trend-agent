...
다만 POC 자동수집과 production 지속수집을 구분한다. POC는 opt-in run, 작은 page/product/request cap, deadline, derived-only artifact를 사용한다. 예약·반복 수집, 장기 raw HTML·JSON·image 보존과 재배포는 source별 production contract 이후 활성화한다. adapter 실패 또는 `manual_only` source는 정상 브라우저 기반 수동 URL-linked observation으로 보완한다. owner가 자신의 계정 credential을 secret store로 제공하면 그 계정으로 정상 로그인해 접근 가능한 콘텐츠는 수집할 수 있다. 다만 CAPTCHA·WAF·Cloudflare challenge·bot 탐지·proxy rotation은 자동·수동 모두 우회하지 않고, robots.txt가 명시적으로 disallow한 경로와 계정 없이 뚫는 paywall·구독은 수집하지 않는다.
...
access_mode: shopify_api|rest_api|static_html|json_ld|hydration|xhr|dynamic_browser|rss|sitemap|owner_login
...
`expected_count`는 조사 참고값이다. 실행 커버리지 계산은 실제 `collected_count`를 사용한다. POC에서는 `public_fetch_reviewed + automated_poc` source를 자동 fetch할 수 있으며 production contract를 필수 선행조건으로 두지 않는다. `blocked`, `paywalled` source는 계정 없이 자동 fetch 계획을 통과할 수 없다. `requires_login=true` source는 `owner_login_available=true`이고 `credential_env_key`가 secret store에 존재할 때만 owner 계정으로 로그인해 자동 fetch 계획을 통과한다. `restricted` source는 계획에서 근거를 명시할 때만 bounded probe로 진행한다. `manual_review_required` 또는 `manual_only` source는 수동 관찰 task로 전환한다. `public_fetch_reviewed`는 공개 접근, robots와 명시적 제한을 검토했다는 POC 정책 판단이며 법적 권리 보장을 의미하지 않는다. robots.txt가 명시적으로 disallow한 경로는 절대 수집하지 않는다. robots와 약관 snapshot이 만료됐거나 hash가 변경되면 재검사하되, 단순 미확인을 영구 금지로 간주하지 않고 bounded probe 또는 manual review 중 하나를 계획에 명시한다. 수집 중 CAPTCHA·WAF·Cloudflare challenge가 나타나면 즉시 중단하고 access status를 갱신한다. owner credential이 없는 로그인·구독 벽이 나타나면 중단한다.

`issued_free_key`는 NAVER/KEXIM처럼 발급된 무료 quota API를 의미하며 허용된다. quota를 넘기면 유료 전환하지 않고 cache 사용 또는 부분 실패로 종료한다. `session_cookie`는 owner가 자신의 계정으로 정상 로그인해 얻은 세션일 때만 허용하며, 탈취·공유·재사용 세션은 금지한다. `signed_token` 위조, mutation endpoint, endpoint brute force는 금지한다. XHR/hydration은 정상 공개 UI가 호출하는 read-only endpoint이고 registry review가 완료된 경우만 허용한다. robots, 약관, endpoint 정책이 충돌하면 가장 엄격한 조건을 적용한다.
...
`solve_cloudflare=False`, proxy rotation 비활성, spider의 `robots_txt_obey=True`, `follow_redirects="safe"`, `max_redirects=3`을 강제한다. 반복 429, CAPTCHA, WAF, Cloudflare challenge, owner credential 없는 로그인·paywall, robots 명시적 block은 중단 신호다. owner credential이 있는 source는 정상 로그인 세션으로 진행한다. Shopify는 collection 중복 요청을 만드는 `ShopifySpider`를 기본값으로 쓰지 않고 공개 `/products.json?page=N&limit=250` bounded pagination을 우선한다.
...
- 탈취·공유·재사용 로그인 세션 수집(owner 본인 계정 정상 로그인은 허용)
...
- CAPTCHA·WAF·Cloudflare challenge 우회 금지
- 차단 우회·proxy rotation·stealth 봇 탐지 회피 금지
- robots.txt 명시적 disallow 경로 수집 금지
- owner 본인 계정 로그인은 허용하되 credential은 secret store에만 두고 로그·보고서·commit에 남기지 않음. 탈취·공유 세션, 계정 없는 paywall·구독 우회는 금지
- URL credentials 금지
- 80/443만 허용
...
- FashionCLIP
- Instagram 자동 수집
- 계정 없는 유료 콘텐츠 우회, CAPTCHA·WAF 우회
- PDF·PowerPoint
- 조직 단위 복잡한 RBAC
- 대규모 범용 crawler
- 다중 지역 분산 worker
