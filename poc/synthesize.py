...
# LLM#2 합성 모델 — sonnet(오너 선택 2026-07-23). LLM#1 corpus는 analyze.MODEL(opus) 유지.
...
def run_synthesis(bundle: dict, prior_weekly: dict) -> tuple[SynthesisOutput, list[dict]]:
    ...
    raw = _call(SYNTH_SYSTEM, json.dumps(payload, ensure_ascii=False),
                SynthesisOutput, model=SYNTH_MODEL)
    return validate_synthesis(raw, bundle, prior_weekly)


def load_prior_weekly(bundle_dict: dict, db_path=None) -> dict:
    """직전 주 concept_weekly 스냅샷 → {concept_id(=label_ko): 직전 행}.

    M4 sqlite driver.get_prior_weeks 실배선(§9). synthesize가 storage보다 먼저 실행되므로
    이번 주 행은 아직 없다 — get_prior_weeks가 진짜 직전 주를 준다. classify()의 fading
    판정(직전 주 존재 여부)이 이 맵으로 결정된다. 빈/부재 db는 {} 반환(첫 주).
    """
    from poc import storage
    db_path = db_path or config.DB_PATH
    prior: dict = {}
    with storage.SqliteDriver(db_path) as driver:
        for cm in bundle_dict["concepts"]:
            label_ko = cm["concept"]["label_ko"]
            rows = driver.get_prior_weeks(storage.concept_id(label_ko), 1)
            if rows:
                prior[label_ko] = rows[0]
    return prior


def synthesize_bundle(bundle_dict: dict, prior_weekly: dict) -> tuple[dict, dict]:
    """번들 dict → LLM#2 합성 → out 저장. 반환: (상태 dict, {concept_id: classification}).

    class_map은 M4 concept_weekly.classification 저장용(weekly.run이 storage.persist에 전달).
    LLM 실패는 격리(§4.4): 번들은 이미 저장됨, 상태에 fallback + class_map은 {}.
    """
    try:
    ...
    class_map = {c.concept_id: c.classification for c in out.claims}
    return {"claims": len(out.claims), "dropped": len(dropped)}, class_map
...
if __name__ == "__main__":
    ...
    _result, _ = synthesize_bundle(bundle_dict, load_prior_weekly(bundle_dict))
