...
# 단어경계 매칭 — wool⊂lambswool, silk⊂silky 과보고 차단 (MDA-2)
_MATERIAL_RE = {m: re.compile(rf"\b{re.escape(m)}\b") for m in MATERIAL_KEYWORDS}


def extract_materials(*texts: str) -> list[str]:
    """tags·title·body_html 등에서 소재 키워드 스캔, 단어경계 적용 (§12.2)."""
    blob = " ".join(t for t in texts if t).lower()
    return [m for m in MATERIAL_KEYWORDS if _MATERIAL_RE[m].search(blob)]
